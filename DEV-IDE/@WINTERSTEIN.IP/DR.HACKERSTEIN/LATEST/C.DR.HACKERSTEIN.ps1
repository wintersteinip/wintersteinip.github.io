...
- A) Browse to Move:
  ...
  2) Moves its contents into C:\@MERGE
  3) If a destination file already exists (same relative path + same name/ext),
     the incoming colliding file is redirected into the next available C:\@MERGE.000X
...
- B) Browse to Restore:
  ...
  - If you select a folder under one of the C:\@MERGE* destinations, it will move it back to its parent as "@MERGE"
...
function Get-NextMergeRoot {
    param(
        [Parameter(Mandatory=$true)][string]$BasePath,   # C:\@MERGE
    ...
    throw "Collision routing failed: all C:\@MERGE.0001..@MERGE.9999 already contain $FileRelPath"
}
...
function Move-WithFileLevelCollisionRouting {
    param(
        ...
        [Parameter(Mandatory=$true)][string]$DestBaseRoot # C:\@MERGE
    )
...
function BrowseForm-Invoke {
    ...
    $lbl.Text = "Move: rename to @MERGE then merge contents into C:\@MERGE*"
    $lbl.AutoSize = $false
    ...
    $btnRestore.Add_Click({
        $folderDialog.Description = "Select a folder under C:\@MERGE* to RESTORE (folder-level action)."
        $res = $folderDialog.ShowDialog()
        if ($res -ne [System.Windows.Forms.DialogResult]::OK) { return }
...
BrowseForm-Invoke `
    -OnMove {
        ...
        # 3) Move contents with file-level collision routing into C:\@MERGE*
