# DR.HACKERSTEIN.v0.01.0.ps1
# REVISED DATE/TIME: 2026-08-21
# Status: Production-Ready Mockup
#requires -RunAsAdministrator
<#
Recovery/Merge Tool (Windows 11) - Revised for WINTERSTEIN IP
- A) Browse to Move:
  1) Select a Source folder.
  2) Select a Destination parent folder.
  3) Validates that Source and Destination are not identical or nested.
  4) Creates <Destination>\@MERGE.
  5) Moves contents from Source to <Destination>\@MERGE.
  6) Collision Policy: If a file exists at the relative path in @MERGE, 
     the incoming file is redirected to <Destination>\@MERGE\MERGE.0001, 0002, etc.
  7) Takes ownership recursively and grants Administrators Full Control.

- B) Browse to Restore:
  - Moves a selected folder under @MERGE back to its parent as "@MERGE".

USAGE:
1) Run this script as administrator.
2) Select "Browse to Move" to perform the merge.
#>
